import Feature1 from '../assets/img/icon-1.svg';
import Feature2 from '../assets/img/icon-2.svg';
import Feature3 from '../assets/img/icon-3.svg';
import Feature4 from '../assets/img/icon-4.svg';
import Feature5 from '../assets/img/icon-5.svg';

import User1 from '../assets/img/user-1.webp';
import User2 from '../assets/img/user-2.webp';
import User3 from '../assets/img/user-3.webp';

import People1 from '../assets/img/people-1.webp';
import People2 from '../assets/img/people-2.webp';
import People3 from '../assets/img/people-3.webp';
import People4 from '../assets/img/people-4.webp';

export const Textos = {
	main: {
		title: 'Sé visible, crece online con Dinno.',
		subtitle: 'Transformamos tu presencia digital en una poderosa herramienta de crecimiento',
		text: 'Bienvenido a Dinno, nos especializamos en el diseño y desarrollo de páginas web que no solo impresionan, sino que también impulsan tu negocio hacia el éxito. ¿Listo para dar el siguiente paso ? '
	},
	about: {
		title: '¡Conoce nuestros planes!',
		text: 'Nuestros servicios de desarrollo web están diseñados para adaptarse a tus necesidades únicas. Desde el diseño de sitios web atractivos hasta la optimización para SEO, en Dinno tenemos lo que necesitas para triunfar en línea.'
	},
	feature: {
		title: 'Maximiza tu presencia online con Dinno',
		text: 'Descubre los beneficios de trabajar con Dinno. Aumenta tu visibilidad, mejora la experiencia del usuario y protege tus datos. Con nosotros, convierte visitantes en clientes satisfechos y asegura un crecimiento continuo para tu negocio en línea.'
	},
	prices: {
		title: 'Precios adaptados a tus necesidades',
		text: 'Descubre nuestros paquetes de precios y lleva tu negocio al siguiente nivel. ¡Calidad, eficiencia y resultados garantizados a precios que te sorprenderán! ¡Elige Dinno y destaca en el mundo digital hoy mismo!'
	},
	conocenos: {
		title: '¿Quienes somos?',
		subtitle: 'Conócenos',
		text: 'En Dinno, somos un equipo apasionado por el diseño y desarrollo de páginas web modernas y accesibles. Nos encanta convertir tus ideas en sitios web increíbles y funcionales.'
	}
}

export const TextServices = {
	title: '¡Tu viaje web comienza aquí!',
	text: '¿Quieres llevar tu negocio al mundo digital pero no sabes por dónde empezar? ¡No te preocupes! Con nuestros paquetes web te guiaremos paso a paso en el emocionante viaje de crear tu propia presencia en línea. Desde diseños simples hasta personalizaciones únicas, tenemos la opción perfecta para ti. ¡Deja que Dinno te ayude a dar el primer paso hacia el éxito en línea!',
	basic: {
		title: 'Presencia web inicial',
		text: 'Tu primer paso hacia el éxito online. Obtén una presencia sólida y profesional en la web para conectar con tu audiencia y expandir tu alcance. Perfecto para quienes dan sus primeros pasos en el mundo digital.'
	},
	enterprise: {
		title: 'Maximiza tu impacto en línea',
		text: 'Lleva tu negocio al siguiente nivel con nuestro Paquete Empresarial. Ofrece funcionalidades avanzadas, diseño personalizado y estrategias de marketing integradas para destacar en línea y atraer a más clientes.'
	},
	premium: {
		title: 'Tu visión, nuestra creación',
		text: 'Transforma tu visión en realidad con nuestro Paquete Personalizado. Trabajamos contigo para crear una solución web adaptada a tu marca y audiencia. Características avanzadas y diseño a medida para una presencia en línea inolvidable.'
	},
	contact: {
		title: '¿Listo para empezar tu viaje digital?',
		text: 'Nuestro equipo está listo para escuchar tus ideas y convertirlas en una experiencia digital impresionante. ¡Déjanos saber cómo podemos ayudarte y comencemos a construir algo increíble!'
	}
}

export const TextFeature = [
	{
		title: 'Aumenta tu presencia online',
		text: 'Destaca entre la competencia, aumenta tu visibilida y llega a más clientes potenciales en línea.',
		image: Feature1
	},
	{
		title: 'Convierte visitantes en clientes satisfechos',
		text: 'Diseñamos sitios web que convierten, aumenta tus ventas y maximiza el retorno de tu inversión en línea.',
		image: Feature2
	},
	{
		title: 'Navegación intuitiva, experiencia inolvidable',
		text: 'Ofrece a tus clientes una experiencia de navegación fluida, cada visita será una experiencia memorable.',
		image: Feature3
	},
	{
		title: 'Seguridad sin compromisos, protegiendo tu negocio',
		text: 'Protege tus datos y la información de tus clientes con nuestras medidas de seguridad de vanguardia.',
		image: Feature4
	},
	{
		title: 'Asistencia permanente, tu éxito es nuestro compromiso',
		text: 'Ofrecemos soporte continuo para que tu sitio web funcione sin problemas en todo momento.',
		image: Feature5
	},
]

export const DataTestimonial = [
	{
		id: 1,
		name: 'María Fernández, Desarrolladora web en formación',
		text: '¡Estoy emocionada con el trabajo de Dinno! Han transformado completamente mi portafolio personal en línea. ¡Estoy muy agradecida!',
		image: User3,
		color: '#ECE5FF',
	},
	{
		id: 2,
		name: 'Carlos González, Emprendedor en busca de clientes',
		text: '¡Estamos encantados con el equipo de Dinno! Gracias a ellos, nuestra landing page ahora está generando más leads. ¡Estamos muy emocionados!',
		image: User1,
		color: '#FBF7B8',
	},
	{
		id: 3,
		name: 'Laura Martínez, Fundadora de una startup tecnológica',
		text: '¡Dinno es increíble! Su enfoque centrado en el cliente y su creatividad nos sorprendieron. ¡Gracias por todo!',
		image: User2,
		color: '#D9FFD8',
	}
]

export const DataQuestions = [
	{
		title: '¿Cuál es la importancia de tener una página web para mi negocio?',
		text: 'Una página web es una herramienta fundamental para cualquier negocio en la era digital. Proporciona una presencia en línea 24/7, permite llegar a una audiencia global, aumenta la visibilidad de la marca, facilita la comunicación con los clientes y ayuda a generar leads y ventas.'
	},
	{
		title: '¿Por qué debería tener un sitio web si ya tengo una página en redes sociales?',
		text: 'Un sitio web te proporciona un espacio exclusivo y personalizable donde puedes mostrar toda la información relevante sobre tu negocio de manera estructurada y profesional. A diferencia de las redes sociales, tienes control total sobre el contenido y el diseño de tu sitio web, lo que te permite transmitir una imagen más completa y auténtica de tu marca.'
	},
	{
		title: '¿Cómo puede ayudarme el posicionamiento web (SEO) a mejorar mi visibilidad en Internet?',
		text: 'El SEO es crucial para mejorar la visibilidad y el ranking de tu sitio web en los motores de búsqueda como Google. Al optimizar tu sitio con palabras clave relevantes, contenido de calidad, enlaces de retroceso y una estructura técnica sólida, puedes aumentar tu tráfico orgánico, atraer a más clientes potenciales y superar a la competencia en línea.'
	},
	{
		title: '¿Cómo puede ayudarme un sitio web a conseguir más clientes?',
		text: 'Un sitio web bien diseñado y optimizado puede aumentar tu visibilidad en línea y hacer que sea más fácil para los clientes potenciales encontrarte cuando buscan productos o servicios similares. Además, con herramientas como formularios de contacto o botones de llamada a la acción, puedes convertir a los visitantes en clientes potenciales y, eventualmente, en clientes reales.'
	},
	{
		title: '¿Cuánto tiempo lleva crear un sitio web y cuánto cuesta?',
		text: 'El tiempo y el costo de crear un sitio web pueden variar según la complejidad del proyecto y los requisitos específicos del cliente. En general, un sitio web básico puede completarse en unas pocas semanas, mientras que un sitio web más complejo puede llevar más tiempo. El costo también dependerá de factores como el diseño, la funcionalidad y el nivel de personalización deseado.'
	}
]

export const DataPrices = [
	{
		type: 'main',
		package: 'Básico',
		price: '5,000',
		items: [
			'Web profesional y responsive',
			'Hasta 3 páginas',
			'SEO básico',
			'Formulario de contacto',
			'Integración de redes sociales',
			'Soporte técnico básico'
		],
		limit: false
	},
	{
		type: 'secondary',
		package: 'Empresarial',
		price: '10,000',
		items: [
			'Web profesional y responsive',
			'Hasta 8 páginas',
			'SEO avanzado',
			'Formularios personalizados',
			'Análisis y seguimiento',
			'Integración de redes sociales',
			'Soporte técnico prioritario'
		],
		limit: false
	},
	{
		type: 'main',
		package: 'Personalizado',
		price: '20,000+',
		items: [
			'Web totalmente a medida y único',
			'Número ilimitado de páginas',
			'SEO avanzado',
			'Funcionalidades personalizadas',
			'Seguridad avanzada',
			'Integración de redes sociales',
			'Soporte técnico 24/7',
			'Actualizaciones regulares'
		],
		limit: false
	}
]

export const PriceHelp = {
	title: '¿Preocupado por el presupuesto?',
	text: 'En Dinno, estamos comprometidos a ofrecerte soluciones de desarrollo web que se ajusten a tus necesidades financieras. Ya sea que estés buscando un sitio web básico, empresarial o personalizado, estamos aquí para adaptarnos a tus requisitos específicos y encontrar la mejor opción para ti. ¡Hablemos y descubre cómo podemos hacer que tu proyecto despegue sin gastar una fortuna!'
}

export const TextAbout = {
	title: '¡Hola amigos!',
	sub: 'Dinno nace de la idea de jóvenes emprendedores que comparten una gran pasión por el desarrollo de páginas web. Desde el principio, nuestra misión ha sido ayudar a las pequeñas empresas a tener una presencia en línea fuerte y efectiva.',
	sub2: 'Nos dimos cuenta de que muchas pequeñas empresas necesitan una página web, pero los altos costos y la complejidad del desarrollo web pueden ser desalentadores. Así que decidimos hacer algo al respecto. En Dinno, creemos que todos deberían tener acceso a un sitio web de calidad, sin importar el tamaño de su presupuesto.',
	sub3: 'Nos encanta trabajar de cerca con nuestros clientes, entendiendo sus necesidades y ofreciendo soluciones personalizadas que no solo son efectivas, sino también asequibles. Queremos ver a las pequeñas empresas crecer y prosperar en el mundo digital, y estamos aquí para hacer que eso suceda.',
	sub4: 'Así que, si estás buscando una manera de llevar tu negocio al siguiente nivel en línea, ¡estamos aquí para ayudarte!',
	sub5: '¡Gracias por confiar en nosotros y permitirnos ser parte de tu viaje digital!',
	peopleTitle: 'Más que un equipo una familia.'
}

export const DataPeople = [
	{
		title: 'Aldo',
		position: 'Finanzas',
		image: People1,
		color: ''
	},
	{
		title: 'Itzel',
		position: 'COO - Operaciones',
		image: People3,
		color: ''
	},
	{
		title: 'Oliver',
		position: 'CEO - Dirección',
		image: People2,
		color: ''
	},
	{
		title: 'Elizabeth',
		position: 'Desarrollador Web',
		image: People4,
		color: ''
	},
]